...
void	ScalarConverter::convert(std::string& input)
{
	...
	{
		if (input.length() == 1 && input[0] >= ' ' && input[0] <= '~' && !std::isdigit(input[0])) // Verify if we are dealing with input like: "a", "b"...
			printTypes(input[0]); // If we so, we can't use strtod(), because it will assume as an input error and will
